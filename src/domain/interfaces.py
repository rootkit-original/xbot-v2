...
class ITechnicalAnalysisService(ABC):
    ...
    async def calculate_bollinger_bands(
        self, candles: List[Candle], period: int = 20, std_dev: int = 2
    ) -> Dict[str, List[Decimal]]:
    ...
    async def calculate_moving_averages(
        self, candles: List[Candle], periods: List[int]
...
class IAIService(ABC):
    ...
    async def predict_price_movement(
        self, candles: List[Candle], timeframe: str, horizon: int = 24  # hours
    ) -> Dict[str, Decimal]:
    ...
    async def optimize_strategy(
        self, strategy: TradingStrategy, performance_history: List[Dict]
    ) -> TradingStrategy:
    ...
    async def detect_anomalies(
        self, market_data: Dict, threshold: Decimal = Decimal("0.95")
    ) -> List[Dict[str, any]]:
