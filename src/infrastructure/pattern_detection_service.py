...
class PatternDetectionService(IPatternDetectionService):
    ...
    async def analyze_pattern_strength(self, pattern: Pattern) -> Decimal:
        ...
        if hasattr(pattern, 'volume_ratio') and pattern.volume_ratio:
            volume_factor = min(Decimal('1.2'), Decimal('1.0') + (pattern.volume_ratio - Decimal('1.0')) * Decimal('0.5'))
    ...
    async def _detect_bullish_engulfing(self, candles: List[Candle]) -> List[Pattern]:
        ...
        for i in range(1, len(candles)):
            ...
            if (prev_candle.is_bearish and curr_candle.is_bullish and
                curr_candle.open_price < prev_candle.close_price and
                curr_candle.close_price > prev_candle.open_price and
                curr_candle.body_size > prev_candle.body_size * Decimal('1.2')):  # Corpo 20% maior
                
                # Calcula confiança baseada no engolfamento
                engulfment_ratio = curr_candle.body_size / prev_candle.body_size
                confidence = min(Decimal('0.9'), Decimal('0.6') + (engulfment_ratio - Decimal('1.0')) * Decimal('0.1'))
                
                # Volume confirmation
                volume_ratio = curr_candle.volume / prev_candle.volume if prev_candle.volume > 0 else Decimal('1.0')
                ...
                pattern = Pattern(
                    ...
                    candles_analyzed=2,
                    key_levels=[prev_candle.low_price, curr_candle.high_price, curr_candle.close_price],
                    is_bullish=True
                )
    ...
    async def _detect_bearish_engulfing(self, candles: List[Candle]) -> List[Pattern]:
        ...
        for i in range(1, len(candles)):
            ...
            if (prev_candle.is_bullish and curr_candle.is_bearish and
                curr_candle.open_price > prev_candle.close_price and
                curr_candle.close_price < prev_candle.open_price and
                curr_candle.body_size > prev_candle.body_size * Decimal('1.2')):
                ...
                pattern = Pattern(
                    ...
                    detected_at=curr_candle.timestamp,
                    candles_analyzed=2,
                    key_levels=[prev_candle.high_price, curr_candle.low_price, curr_candle.close_price],
                    is_bearish=True
                )
    ...
    async def _detect_hammer(self, candles: List[Candle]) -> List[Pattern]:
        ...
        for candle in candles[-10:]:  # Últimos 10 candles
            ...
            if (lower_shadow >= body_size * Decimal('2.0') and  # Sombra inferior >= 2x corpo
                upper_shadow <= body_size * Decimal('0.5') and  # Sombra superior <= 0.5x corpo
                body_size > Decimal('0')):  # Tem corpo
    ...
    async def _detect_doji(self, candles: List[Candle]) -> List[Pattern]:
        ...
        for candle in candles[-5:]:  # Últimos 5 candles
            ...
            if (total_range > 0 and 
                body_size <= total_range * Decimal('0.1')):  # Corpo <= 10% do range total
    ...
    async def _detect_support_resistance(self, candles: List[Candle]) -> List[Pattern]:
        ...
        nearby_support = [s for s in support_levels 
                         if s < current_price and s >= current_price * Decimal('0.95')]
    ...
    async def _detect_breakout(self, candles: List[Candle]) -> List[Pattern]:
        ...
        if range_size < avg_price * Decimal('0.05'):  # Range < 5% do preço médio
            ...
            if (current_candle.close_price > resistance_level and
                current_candle.volume > sum(c.volume for c in consolidation_candles[-5:]) / Decimal('5') * Decimal('1.5')):
            ...
            elif (current_candle.close_price < support_level and
                  current_candle.volume > sum(c.volume for c in consolidation_candles[-5:]) / Decimal('5') * Decimal('1.5')):
    ...
    def _find_pivot_points(self, values: List[Decimal], is_high: bool, window: int = 5) -> List[Decimal]:
