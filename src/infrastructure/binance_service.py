...
class BinanceService(
    IMarketDataRepository, ITradingRepository, IPortfolioRepository, IMarketDataStream
):
    ...
    async def place_order(self, order: Order) -> Order:
        ...
        try:
            ...
            if "fills" in data:
                executed_value = sum(
                    Decimal(fill["price"]) * Decimal(fill["qty"]) for fill in data["fills"]
                )
            ...
            for order_data in data:
                order = Order(
                    ...
                    price=(
                        ...
                        if order_data["price"] != "0.00000000"
                        else None
                    ),
