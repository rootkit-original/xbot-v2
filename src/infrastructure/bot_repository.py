...
class InMemoryBotRepository(IBotRepository):
    ...
    async def get_bot_performance(self, bot_id: str, days: int = 30) -> Dict:
        ...
        recent_orders = [
            order for order in bot.order_history
        ...
        total_pnl = sum(
            order.realized_pnl for order in recent_orders
    ...
    async def get_portfolio_summary(self) -> Dict:
        ...
        total_unrealized_pnl = sum(
            sum(pos.unrealized_pnl for pos in bot.active_positions)
    ...
    async def backup_bots(self, backup_path: str) -> bool:
        ...
        try:
            ...
            backup_file = backup_dir / f"xbot_backup_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
