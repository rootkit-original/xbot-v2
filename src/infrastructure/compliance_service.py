...
class ComplianceService(IComplianceService):
    ...
    async def validate_trade(
        self, order: Order, current_positions: List[Position]
    ) -> Tuple[bool, str]:
        ...
        existing_position = next(
            (pos for pos in current_positions if pos.symbol == order.symbol), None
        )
    ...
    async def check_position_limits(self, bot: TradingBot) -> Tuple[bool, str]:
        ...
        for position in bot.active_positions:
            ...
            if position_percentage > max_position_size:
                return (
                    False,
                    f"Posição {position.symbol} excede limite: {position_percentage:.1f}% > {max_position_size}%",
                )
        ...
        if total_exposure > max_total_exposure:
            return (
                False,
                f"Exposição total excede limite: ${total_exposure} > ${max_total_exposure}",
            )
    ...
    async def check_daily_loss_limit(self, bot: TradingBot) -> Tuple[bool, str]:
        ...
        if current_loss_percentage > daily_loss_limit:
            return (
                False,
                f"Limite de perda diária excedido: {current_loss_percentage:.2f}% > {daily_loss_limit:.2f}%",
            )

        # Check unrealized losses
        unrealized_loss = sum(min(pos.unrealized_pnl, Decimal("0")) for pos in bot.active_positions)
        ...
        if unrealized_loss != 0:
            ...
            if unrealized_loss_percentage > daily_loss_limit:
                return (
                    False,
                    f"Perdas não realizadas excedem limite: {unrealized_loss_percentage:.2f}% > {daily_loss_limit:.2f}%",
                )
    ...
    async def check_risk_management_rules(self, bot: TradingBot) -> List[str]:
        ...
        if bot.total_trades > 20 and bot.win_rate < Decimal("30"):  # 30% minimum win rate
            violations.append(
                f"Taxa de acerto baixa: {bot.win_rate:.1f}% ({bot.total_trades} trades)"
            )
            ...
            for position in bot.active_positions:
                if total_value > 0:
                    ...
                    if concentration > 50:  # No single position > 50% of portfolio
                        violations.append(
                            f"Concentração excessiva em {position.symbol}: {concentration:.1f}%"
                        )
    ...
    async def get_audit_logs(self, bot_id: Optional[str] = None, hours: int = 24) -> List[Dict]:
        """Obtém logs de auditoria"""

        cutoff_time = datetime.now() - timedelta(hours=hours)

        filtered_logs = [log for log in self.audit_logs if log["timestamp"] >= cutoff_time]

        if bot_id:
            filtered_logs = [log for log in filtered_logs if log["bot_id"] == bot_id]
