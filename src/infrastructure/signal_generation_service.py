...
from ..domain.interfaces import ISignalGenerationService, IPatternDetectionService, ITechnicalAnalysisService
...
class SignalGenerationService(ISignalGenerationService):
    ...
    async def generate_signal(
        ...
        relevant_patterns = [
            p for p in market_analysis.detected_patterns
            if p.pattern_type in strategy.preferred_patterns and
            p.confidence >= strategy.min_pattern_confidence
        ]
        ...
        logger.info(f"Sinal gerado: {signal_type} {market_analysis.symbol} - Confiança: {final_confidence:.2%}")
        return signal
    ...
    async def _calculate_technical_score(self, analysis: MarketAnalysis) -> Decimal:
        ...
        if analysis.moving_averages:
            ma_score = self._evaluate_moving_averages(analysis.moving_averages, analysis.current_price)
