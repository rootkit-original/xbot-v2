...
from application.use_cases import (
    AnalyzeMarketUseCase, DetectPatternsUseCase,
    ExecuteTradeUseCase, RunTradingBotUseCase
)
            ...
            'patterns': [
                {
                    ...
                    'direction': 'bullish' if p.is_bullish else 'bearish' if p.is_bearish else 'neutral'
                }
...
async def main():
    ...
    parser.add_argument('--create-config', action='store_true', help='Cria template de configuração')
    parser.add_argument('--validate-config', action='store_true', help='Valida configuração')
    parser.add_argument('--list-strategies', action='store_true', help='Lista estratégias disponíveis')
    
    # Bot management commands
    parser.add_argument('--create-bot', type=str, help='Cria novo bot com nome especificado')
    parser.add_argument('--strategy', type=str, default='conservative', help='Estratégia para o bot')
    ...
    try:
        ...
        if args.create_bot:
            ...
            if bots:
                ...
                for bot in bots:
                    status_emoji = "🟢" if bot.status == BotStatus.IDLE else "🔴" if bot.status == BotStatus.ERROR else "🟡"
            ...
            if analysis['patterns']:
                for pattern in analysis['patterns']:
                    print(f"    - {pattern['type']}: {pattern['confidence']:.1%} ({pattern['direction']})")
