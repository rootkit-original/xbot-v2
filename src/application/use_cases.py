...
from domain.entities import (
    ...
    TradingSignal, Candle, BotStatus, OrderSide, OrderType, PatternType
)
from domain.interfaces import (
    IMarketDataRepository, ITradingRepository, IPortfolioRepository, IBotRepository,
    IPatternDetectionService, ITechnicalAnalysisService, IRiskAnalysisService,
    ISignalGenerationService, INotificationService, IComplianceService,
    IMarketDataStream, IAIService
)
...
class AnalyzeMarketUseCase:
    ...
    async def execute(
        ...
        ma_dict = {f"MA{period}": values[-1] for period, values in moving_averages.items() if values}
        ...
        liquidity_score = self._calculate_liquidity_score(main_candles, stats_24h.get('volume', Decimal('0')))
    ...
    async def execute(
        ...
        try:
            ...
            await self.notification_service.send_trade_notification(
                bot.id,
                executed_order,
                f"Trade executado: {signal.signal_type} {signal.symbol}"
            )
    ...
    async def execute(self, bot: TradingBot) -> List[Tuple[Position, bool, str]]:
        ...
        for position in positions:
            ...
            if candles:
                ...
                # Deve fechar posição?
                should_close, reason = await self.risk_analyzer.should_close_position(position, analysis)
    ...
    async def execute(self, bot_id: str) -> Dict[str, any]:
        ...
        try:
            ...
            await self.notification_service.send_risk_alert(
                bot, f"Erro crítico no bot: {str(e)}"
            )
